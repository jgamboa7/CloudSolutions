...
containers:
  ...
  {{- if .Values.plugins }}
    ...
    #   - "fluentd"
    #   - "-c"
    #   - "/etc/fluent/fluent.conf"   # <-- ABSOLUTE PATH TO YOUR MOUNTED CONFIG
    #   - "-p"
    #   - "/fluentd/plugins"          # <-- Plugin path from original image
    #   - "--gemfile"
    #   - "/fluentd/Gemfile"          # <-- Gemfile path from original image
    - "/bin/sh"
    - "-c"
    - |
  ...
  {{- end }}
    ...
    - name: etcfluentd-main
      mountPath: /fluentd/etc/fluent.conf # /etc/fluent
      subPath: fluent.conf
    - name: etcfluentd-config
      mountPath: /fluentd/etc/config.d/ # /etc/fluent/config.d/
